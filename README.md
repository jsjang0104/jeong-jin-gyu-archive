...
```python
import sys
sys.path.insert(0, '/home/<username>/jeong-jin-gyu-archive/backend')
...
**환경변수 설정**: `backend/.env`에 다음을 작성합니다:

```
SECRET_KEY=your-arbitrary-random-secret-key-here
DEBUG=false
```
...
Vercel(또는 다른 Frontend 배포 플랫폼)에서 환경변수 `VITE_API_URL`을 다음으로 설정합니다:
...
- **외부 API 화이트리스트**: 무료 플랜은 외부 API 호출이 화이트리스트로 제한되므로, Gemini API 호출이 차단될 수 있습니다. 이 경우 AI 자동 등록 기능(`/manage/ai-upload`)은 자동으로 예시 응답 모드(`"demo": true`)로 동작합니다.
- **SQLite 및 미디어 저장소**: 파일시스템이 영구 저장되므로 Neon 같은 외부 데이터베이스는 불필요합니다. 원하면 `DATABASE_URL` 환경변수로 Neon PostgreSQL을 연결할 수도 있습니다.

### 대안: Hugging Face Spaces (Docker)

2026년부터 Hugging Face Docker SDK가 유료화되므로, 새로운 배포는 PythonAnywhere를 권장합니다. 기존 HF Spaces 배포 방식도 계속 지원합니다.
...
기존 Render 배포 방식도 계속 지원합니다(`render.yaml` 유지). Render 무료 플랜은 일정 시간 미사용 시 슬립되고 리소스 한도가 있으므로 PythonAnywhere를 기본값으로 권장하지만, 필요하면 아래 방식으로 그대로 배포할 수 있습니다.
